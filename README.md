...
| Tools | 23 tools mirrored 1:1 from [mcp-atlassian](https://github.com/sooperset/mcp-atlassian) upstream |
...
AI Agent (Claude Code / Cursor)
    |
    |-- POST /rest/mcp/1.0/ --> MCP Endpoint (Streamable HTTP)
    |                               |
    ...
    |                                               +--> ResponseTrimmer
    |                                               |
    |                                               +--> Confluence REST API (localhost)
    |
    |-- OAuth 2.0 --> OAuth Proxy (/plugins/servlet/mcp-oauth/*)
                          |
...
```bash
# 1. Install: download JAR from Releases, upload via UPM
...
The plugin implements MCP Streamable HTTP on a single endpoint.
...
```
MCP Client --> Plugin OAuth Proxy --> Confluence OAuth 2.0 --> Consent --> Token --> Done
```
...
The plugin runs inside the Confluence JVM. No data leaves your infrastructure.
...
| Rate limiting | IP-based rate limiter (register 5/min, token 20/min, MCP 120/min) |
| Body limits | 1MB for MCP requests, 64KB for register, 8KB for token |
...
> [!CAUTION]
> The plugin makes localhost HTTP calls to Confluence's own REST API. No outbound network connections are made.
...
Confluence's REST API returns verbose JSON that AI agents don't need. The plugin strips noise before returning results (83% size reduction).
