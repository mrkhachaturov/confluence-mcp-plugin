 ...
 *
 * <p>Pipeline: 1. Pre-process: convert extended Markdown syntax to Confluence XML macros 2. Parse
 * with flexmark (Confluence XML passes through untouched) 3. Post-process: convert flexmark task
 * list HTML to native ac:task-list
 *
 * <p>Flexmark is configured with two key settings: - Parser.HTML_ALLOW_NAME_SPACE = true ->
 * recognizes ac and ri namespaced tags as valid HTML - Parser.HTML_BLOCK_TAGS += Confluence tags ->
 * Type 6 block detection
 *
 * <p>This means all <ac:structured-macro>, <ac:parameter>, etc. pass through flexmark as raw HTML
 * -- no placeholders, no escaping, no post-processing needed.
 *
 * <p>Supported extensions beyond standard GFM: - GitHub alerts: > [!NOTE], > [!TIP], >
 * [!IMPORTANT], > [!WARNING], > [!CAUTION] - Status labels: {status:Text} or {status:Text|color} -
 * Table of contents: {toc} or [TOC] - Task lists: - [x] done, - [ ] todo -> native ac:task-list -
...
public final class MarkdownToStorage {
  ...
  static {
    ...
    List<com.vladsch.flexmark.util.misc.Extension> extensions =
        List.of(
  ...
  private static final Pattern GITHUB_ALERT =
      Pattern.compile(
          "^> *\\[!(NOTE|WARNING|TIP|IMPORTANT|CAUTION)(?:\\|title:([^\\]]*))?\\]\\s*$",
          Pattern.MULTILINE | Pattern.CASE_INSENSITIVE);

  /** Status labels: {status:Text} or {status:Text|color} */
  private static final Pattern STATUS_LABEL =
      Pattern.compile("\\{status:([^|}]+)(?:\\|([^}]+))?\\}");

  /** Table of contents: {toc} or [TOC] on its own line */
  private static final Pattern TOC_MARKER =
      Pattern.compile("^(?:\\{toc\\}|\\[TOC\\])$", Pattern.MULTILINE | Pattern.CASE_INSENSITIVE);

  /** HTML details/summary -> Confluence expand macro */
  private static final Pattern DETAILS_BLOCK =
      Pattern.compile("<details>\\s*<summary>(.*?)</summary>(.*?)</details>", Pattern.DOTALL);
  ...
  private static final Pattern TASK_LIST_UL =
      Pattern.compile(
   ...
   *
   * <p>Input: Output: > [!NOTE] <ac:structured-macro ac:name="info"> > This is important.
   * <ac:rich-text-body>
   *
   * <p>This is important.</ac:rich-text-body> </ac:structured-macro>
   */
  ...
  /**
   * Convert {status:Text|color} to inline Confluence status macro. Inline XML works because
   * HTML_ALLOW_NAME_SPACE = true.
   */
  private static String convertStatusLabels(String markdown) {
    ...
    while (m.find()) {
      ...
      if (color != null && !color.isBlank()) {
        String normalized =
            color.trim().substring(0, 1).toUpperCase() + color.trim().substring(1).toLowerCase();
        macro
            .append("<ac:parameter ac:name=\"colour\">")
            .append(normalized)
            .append("</ac:parameter>");
      }
    ...
    while (ulMatcher.find()) {
      ...
      while (itemMatcher.find()) {
        ...
        taskList
            .append("<ac:task>")
            .append("<ac:task-id>")
            .append(taskIdCounter++)
            .append("</ac:task-id>")
            .append("<ac:task-status>")
            .append(checked ? "complete" : "incomplete")
            .append("</ac:task-status>")
            .append("<ac:task-body>")
            .append(body)
            .append("</ac:task-body>")
