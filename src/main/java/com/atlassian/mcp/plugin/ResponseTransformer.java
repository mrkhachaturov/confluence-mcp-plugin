...
/**
 * Transforms raw Confluence API JSON responses into the simplified format that upstream
 * mcp-atlassian returns via its Pydantic models + to_simplified_dict().
 *
 * <p>Each method mirrors a specific upstream model's transformation.
 */
public final class ResponseTransformer {
  ...
  /**
   * Build a full page URL for Server/DC. Mirrors upstream:
   * f"{base_url}/pages/viewpage.action?pageId={page_id}"
   */
  ...
  /**
   * Transform a raw Confluence page JSON into upstream's ConfluencePage.to_simplified_dict()
   * format.
   *
   * <p>Input: raw Confluence REST API /rest/api/content/{id} response (after ResponseTrimmer)
   * Output: {id, title, type, created, updated, url, space, author, version, attachments, content,
   * ancestors}
   */
  ...
  public static ObjectNode simplifyPageNode(
      JsonNode raw, String baseUrl, boolean convertToMarkdown) {
  ...
  /**
   * Transform a raw Confluence comment JSON into upstream's ConfluenceComment.to_simplified_dict()
   * format.
   *
   * <p>Output: {id, body, created, updated, author, parent_comment_id, location}
   */
  ...
  /**
   * Transform a raw Confluence label JSON into upstream's ConfluenceLabel.to_simplified_dict()
   * format.
   *
   * <p>Output: {id, name, prefix, label}
   */
   ...
   *
   * <p>Output: {display_name, email, profile_picture}
   */
   ...
   *
   * <p>Output: {id, type, status, title, media_type, file_size, download_url}
   */
  ...
  /**
   * Format a timestamp to "YYYY-MM-DD HH:MM:SS" matching upstream's TimestampMixin. Handles ISO
   * 8601 variants: Z, +0000, +00:00, +03:00, etc. Returns original string if parsing fails.
   */
    ...
    try {
      ...
      if (normalized.contains("+") && !normalized.endsWith("Z")) {
        ...
        if (tzPart.length() == 5 && !tzPart.contains(":")) {
          normalized =
              normalized.substring(0, plusPos) + tzPart.substring(0, 3) + ":" + tzPart.substring(3);
        }
      }
