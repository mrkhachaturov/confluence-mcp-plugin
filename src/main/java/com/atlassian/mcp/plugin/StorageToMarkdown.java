...
/**
 * Converts Confluence storage format (XHTML) to Markdown. Mirrors upstream mcp-atlassian's
 * process_html_content() pipeline: 1. Parse storage format with jsoup (XML mode for ac:/ri:
 * namespaces) 2. Transform all Confluence-specific elements (ac:*, ri:*) into standard HTML 3.
 * Convert standard HTML to Markdown via flexmark-html2md-converter
 *
 * <p>Upstream uses BeautifulSoup + markdownify (Python). We use jsoup + FlexmarkHtmlConverter
 * (Java).
 *
 * <p>Full Confluence storage format support per:
...
public final class StorageToMarkdown {
  ...
  private static void transformExpandMacro(Element macro) {
    ...
    String html = "<details><summary>" + escapeHtml(title) + "</summary>" + bodyHtml + "</details>";
