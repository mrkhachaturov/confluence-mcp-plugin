...
public class ConfigResource {
      ...
      if (!override.isBlank() && !UrlSafety.isSafeBaseUrlForSet(override)) {
        return Response.status(Response.Status.BAD_REQUEST)
            .entity(
                Map.of(
                    "error",
                    "Invalid confluenceBaseUrl: must be an http(s) URL that does "
  ...
  public Response searchUsers(@QueryParam("q") String q) {
    ...
    try {
      ...
      return Response.serverError()
          .entity(Map.of("error", "User search failed"))
          .type(MediaType.APPLICATION_JSON)
          .build();
    }
  ...
  /**
   * Typeahead for the admin "Allowed Groups" picker. Searches group names via the native {@link
   * UserAccessor}. Returns up to {@link #PICKER_LIMIT} {@code {value, label}} entries.
   */
  @GET
  @Path("/groups")
  @Produces(MediaType.APPLICATION_JSON)
  public Response searchGroups(@QueryParam("q") String q) {
    ...
    try {
      for (Group g :
          userAccessor
              .findGroups(new GroupNameTermQuery(term, TermQuery.SUBSTRING_CONTAINS))
              .pager()) {
      ...
      return Response.serverError()
          .entity(Map.of("error", "Group search failed"))
          .type(MediaType.APPLICATION_JSON)
          .build();
    }
  ...
  /**
   * True if the request is same-origin with this Confluence instance (or carries no Origin/Referer,
   * i.e. is not a cross-site browser request). Used as the CSRF gate.
   */
