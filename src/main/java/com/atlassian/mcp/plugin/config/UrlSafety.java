 ...
 * host (credential exfiltration + internal SSRF). These checks bound that risk; the admin REST
 * is also CSRF-protected so the override can no longer be set by a forged cross-site request.
 *
...
public final class UrlSafety {
    ...
    /**
     * Read-time, no-DNS guard. http/https only, must have a host, no embedded credentials,
     * and the host must not be a loopback/metadata literal. Cheap enough to run on every request.
     */
    ...
    /**
     * Admin set-time guard: syntactic check plus DNS resolution that rejects loopback,
     * link-local, any-local, and the cloud-metadata address. Returns false if the host does
     * not resolve (an admin can correct it and retry).
     */
