 ...
 * Content-Length} header (spec §6.3). A fixed-length header over the cap is rejected as a fast
 * path; otherwise the stream is drained up to the cap + 1 byte and, if more remains, the request is
 * rejected with 413. The already-read bytes are re-wrapped so the SDK transport downstream reads
 * the body intact.
 */
@UnrestrictedAccess
