...
/**
 * Filter-based transport for the MCP SDK servlet. Atlassian {@code <servlet>} modules hardcode
 * {@code asyncSupported=false}; {@code <servlet-filter>} modules honor the JVM flag {@code
 * atlassian.plugins.filter.async.default=true}. This filter owns {@code /plugins/servlet/mcp},
 * calls the SDK transport servlet's {@code service(...)} directly, and never invokes {@code
 * chain.doFilter()} — it IS the endpoint.
 */
@UnrestrictedAccess
