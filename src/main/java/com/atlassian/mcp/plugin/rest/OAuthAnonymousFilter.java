 ...
 * (handled by OAuthServlet) 2. Directly serves /.well-known/oauth-* responses (can't use servlets
 * at root) 3. Passes /plugins/servlet/mcp through Confluence's login layer so the MCP filter
 * chain's AccessControlFilter can return a JSON 401 instead of an HTML login redirect.
 */
@UnrestrictedAccess
