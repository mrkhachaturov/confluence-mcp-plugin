 ...
 * unresolved-user requests). Runs before {@link AccessControlFilter}, and §4.1 deliberately lets
 * unauthenticated traffic reach the chain, so the anonymous case gets a real per-IP bucket rather
 * than a shared null key (spec §6.3). Over-limit returns 429 with {@code RateLimit-*} + {@code
 * Retry-After} headers.
 */
@UnrestrictedAccess
...
public class RateLimitFilter implements Filter {
  ...
  private static String resolveBucket(HttpServletRequest req) {
    try {
      ...
      log.debug("could not resolve the authenticated user, falling back to the IP bucket", e);
    }
