...
public class RateLimiter {
  ...
  /**
   * Snapshot of current rate-limit state for a key + endpoint, used to emit RateLimit-* response
   * headers per draft-ietf-httpapi-ratelimit-headers-09.
   */
  ...
  /**
   * Read-only inspection of the current window state. Does NOT consume a slot. {@code resetSeconds}
   * is the time until this key's window rolls over.
   */
  ...
  /**
   * Lazy eviction: only runs when at/over the cap. First drops keys whose window has expired; if
   * still full (all keys active), drops the single oldest-started key. Never rejects callers.
   */
