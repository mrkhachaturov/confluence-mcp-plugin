...
/**
 * Enforces the security invariant: an {@code MCP-Session-Id} issued to one Confluence user may not
 * be used by another. Cross-user replay returns 403; unknown/expired sessions on non-initialize
 * POSTs return 401. Captures the session id from the {@code initialize} response and binds it to
 * the authenticated user; DELETE removes the binding on success.
 */
@UnrestrictedAccess
  ...
  public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
    ...
    if (!isInitialize && incomingSid != null) {
      ...
      if (!b.username().equals(currentUser)) {
        log.warn(
            "[MCP-SEC] session-user binding violation: bound to {} but request from {}",
            b.username(),
            currentUser);
