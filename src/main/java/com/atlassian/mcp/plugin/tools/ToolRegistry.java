...
public class ToolRegistry {
  ...
  /**
   * Adapts the currently-visible tools to MCP SDK {@code SyncToolSpecification}s for registration
   * on the sync server. Applies the same three filters as {@link #listTools}: capability gate,
   * admin-disabled list, and read-only-hides-write (spec §6.4).
   */
  ...
  public String checkToolAccess(String toolName, String userKey) {
    ...
    if (!isCapabilityMet(tool)) {
      return "Tool '"
          + toolName
          + "' requires a Confluence plugin that is not installed on this instance";
    }
